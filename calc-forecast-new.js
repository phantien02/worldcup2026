...
async function forecast() {
  ...
  const INITIAL_FEE = 1534000;
  const POINT_VALUE = 2000;
  const GOAL_AVG = 500000;
  
  const MAX_SO_FAR = 384; 
  const MAX_TOTAL = 1534;
  ...
  const playersStats = profiles.map(p => {
    ...
    const currentRevenue = INITIAL_FEE - (p.total_points * POINT_VALUE);
    const forecastRevenue = INITIAL_FEE - (forecastScore * POINT_VALUE);
    ...
    return {
      ...
      forecastScore,
      playerProfit: (forecastScore * POINT_VALUE) - INITIAL_FEE,
      btcRevenue: forecastRevenue
    };
  }).sort((a, b) => b.forecastScore - a.forecastScore);
  
  const currentBtcRevenue = (PLAYERS_COUNT * INITIAL_FEE) - (currentTotalPoints * POINT_VALUE);
  const forecastBtcRevenue = (PLAYERS_COUNT * INITIAL_FEE) - (forecastTotalPoints * POINT_VALUE);
